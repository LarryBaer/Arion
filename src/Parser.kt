...
class Parser(private val tokenList: List<Token>) {
    ...
    private fun parseForStatement(): Statement {
        ...
        val expression1: Statement =
            if (compareNextToken("variable")) {
                scanToken()
                parseVariableDeclaration()
            } else {
        ...
        loopBody =
            Section(
                listOf(
                    expression1,
                    While(expression2, Section(listOf(loopBody, ExpressionStatement(expression3))))
                )
            )
    ...
    private fun parseSection(): List<Statement>? {
        ...
        while (
            ...
            tokenList[currentIndex].tokenType != "right_curly_brace"
        ) {
    ...
    private fun validateSyntax(tokenTypeInput: String) {
        ...
        throw Exception(
            "Syntax validation error with type: $tokenTypeInput and peek: " +
